...
<br><br><br>
  ...
  console.log("--"+strResult); // display text on the console
  var hexInDec = parseInt(strResult, 16);
  console.log("--"+hexInDec); // display text on the console
  //hexHundred = "100".toString(16);
  var valueFinal = 256 - hexInDec; //256 is 100 in hex
  //console.log(parseInt(strResult, 16))
//console.log(parseInt("64", 16))
console.log("Final value into file: "+valueFinal);
var finalValueHex = parseInt(valueFinal, 16);
return valueFinal;
//console.log(finalValueHex);
  
  //console.log(hexHundred-hexString);
    ...
    rawFile.onreadystatechange = function ()
    {
        if(rawFile.readyState === 4) // readyState = 4: request finished and response is ready
        {
            if(rawFile.status === 200) // status 200: "OK"
            {
				...
				///////////
                console.log("ALL TEXT: "+allText); // display text on the console
				
				 var result = allText;
				 //start of 80Hex, will produce fffd
                var hex = "";
...
console.log("-----------");
                ...
                for (var i = 0; i < result.length; i++) {
                    var byteStr = result.charCodeAt(i);
                    if (byteStr.length < 32) {
                        byteStr = "0" + byteStr;
						console.log(" Result : "+byteStr);
                    }else
					{
						byteStr = "0" + byteStr;
				...
				//calc_cksum8("01 00 00 01 0D 19 00 D8 01 0C 19 87 22 C9 C8 42 72 6F 61 64 63 6F 6D DB 42 52 43 4D 20 31 30 30 47 20 31 50 20 35 37 35 30 34 20 4F 43 50 20 4D 65 7A 7A D2 4D 31 31 30 30 47 30 31 39 32 30 41 30 30 33 32 43 51 D0 42 43 4D 39 35 37 35 30 34 2D 4D 31 31 30 30 47 CA 31 32 2F 31 37 2F 32 30 32 30 C1 00 00 00 00 5B 01 0C 19 C8 42 72 6F 61 64 63 6F 6D DB 42 52 43 4D 20 31 30 30 47 20 31 50 20 35 37 35 30 34 20 4F 43 50 20 4D 65 7A 7A D0 42 43 4D 39 35 37 35 30 34 2D 4D 31 31 30 30 47 CB 32 31 36 2E 32 2E 32 37 31 2E 31 D2 4D 31 31 30 30 47 30 31 39 32 30 41 30 30 33 32 43 51 00 00 C1 00 00 00 00 99 C0 82 05 B0 09 3D 11 00 01 01 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00");
				  //alert(calc_cksum8(hex)); 
				  //hex="00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F A0 B0 C0 D0 E0 F0";
				  //hex="00";
...
readTextFile(<?php echo "'project_build/logitech/loigc_01/Working Folder/support/a.bin'"; ?>); //<= Call function ===== don't need "file:///..." just the path   
