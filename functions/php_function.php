...
function checksumExtraUpload($target_file_bin, $locStart="", $locEnd="", $textAreaName){  
global $con;
...
$boardValueHexNow = "";
	...
	$decValStart = hexdec($locStart);
	
	 
	
	$decVal = hexdec($boardValueCheckHexReal[1]);
	$decVal = hexdec($locEnd);
	...
	$length = $decVal - $decValStart; 
		...
		echo $checkSumToCompare = substr(getHexFromBinFile($target_file_bin),(dechex($locStart)*3),2);
       ...
       // echo  '</pre>';
		...
		echo "CHECK SUM FROM BIN FILE <input id='".$textAreaName."bin' type='text' value='".$checkSumToCompare."' readonly /><br>";
		echo "<br><br>";
		echo "CHECK SUM CALCULATED<input id='".$textAreaName."cal' type='text' value='' readonly /><br>";
		echo "<br><br>";
		echo "<textarea  type='text' id='".$textAreaName."' name='".$textAreaName."' readonly rows='8' cols='80' value='"; echo implode(" ",$newArrayBoardStart); echo "'>".implode(" ",$newArrayBoardStart)."</textarea>";
		echo "<br><br>"; 
		echo "<p id='".$textAreaName."note'></p>";
		
}

function checksumExtra($target_file_fru, $target_file_bin, $locStart="", $locEnd="", $textAreaName){  
global $con;
$read=true;
$boardValueHexNow = "";
 ...
 $file = $target_file_fru;
	...
	$length = $decVal - $decValStart; 
		...
		$checkSumToCompare = substr(getHexFromBinFile($target_file_bin),(dechex($locStart)*3),2);
...
function boardChecksum($target_file_fru, $target_file_bin, $locStart="", $locEnd="", $locChkSum=""){  
global $con;
$read=true;
$boardValueHexNow = "";
 ...
 $file = $target_file_fru;
	...
	$length = $decVal - $decValStart; 
		...
		{
			$locStart = hexdec($locStart);
			$locEnd = hexdec($locEnd);
			$locChkSum = hexdec($locChkSum);
			//$locChkSum = 309; 
			$newArrayBoardStart = array_splice($hexes, $locStart, $locEnd);
			$checkSumToCompare = substr(getHexFromBinFile($target_file_bin),$locChkSum,2);
		}
...
foreach($new_data as $tablecolumn => $datas){
	...
	/*Check exiting data*/
	$query_check = "SELECT * FROM $table WHERE $return_col = '$datas'";
	$sql_check = mysqli_query($con, $query_check);
	if(mysqli_num_rows($sql_check)>0){
		//Data exists
		$row = mysqli_fetch_array($sql_check);
		$data[$data_key] = $row[$return_col];
	}else{
