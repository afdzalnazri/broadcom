...
//----------------------------------------------------------------------------------------------------------------//
$names = explode("|", $_SESSION["allConfig"]);
						$buildConfigOld = $names[0];
						$familyIdOldOld = $names[1];
						$projectIdOld = $names[2];


$sqlProjName = mysqli_query($con,"SELECT * FROM buildconfig WHERE buildconfigname = '$buildConfigOld'");
$row = mysqli_fetch_array($sqlProjName);
$bcIds = $row['ids'];

	
$sqlProjName = mysqli_query($con,"SELECT * FROM buildconfigparam WHERE bcId = '$bcIds' order by id desc limit 1");
$row = mysqli_fetch_array($sqlProjName);
$revOld = $row['value'];

$sqlProjName = mysqli_query($con,"SELECT * FROM buildconfig WHERE id = '$id'");
$row = mysqli_fetch_array($sqlProjName);
$projectIdDB = $row['projectId'];


//echo "SELECT * FROM project WHERE id = '$projectIdDB'";
$sqlProjName = mysqli_query($con,"SELECT * FROM project WHERE id = '$projectIdDB'");
$row = mysqli_fetch_array($sqlProjName);
$productNameOld = $row['productName'];
...
$sqlProjName = mysqli_query($con,"SELECT * FROM buildconfigparam WHERE bcId = '$bcIds' order by id asc limit 1");
$row = mysqli_fetch_array($sqlProjName);
$partNumber = $row['value'];
						
$sqlProjName = mysqli_query($con,"SELECT * FROM project WHERE id = '$projectIdOld'");
$row = mysqli_fetch_array($sqlProjName);
$productName = $row['productName'];


$sqlFamilyName = mysqli_query($con,"SELECT * FROM masterFamily WHERE id = '$familyIdOldOld'");
$row = mysqli_fetch_array($sqlFamilyName);
$familyName = $row['familyName'];

$releaseFolderName  = $productName."-M0_".$revOld;
$testLogFolderName  = $productName."-M0_".$revOld." Test Logs";

$releaseFolderNameNew  = $productName."-M0_".$revNew;
$testLogFolderNameNew  = $productName."-M0_".$revNew." Test Logs";


$oldnameRelFolder = $familyName."\\".$productName."\\".$buildConfigOld."\\".$releaseFolderName;
$oldnameTestFolder = $familyName."\\".$productName."\\".$buildConfigOld."\\".$testLogFolderName;

$newnameRelFolder = $familyName."\\".$productName."\\".$buildConfigOld."\\".$releaseFolderNameNew;
$newnameTestFolder = $familyName."\\".$productName."\\".$buildConfigOld."\\".$testLogFolderNameNew;

rename("..\\..\\project_build\\".$oldnameRelFolder, "..\\..\\project_build\\".$newnameRelFolder);
rename("..\\..\\project_build\\".$oldnameTestFolder, "..\\..\\project_build\\".$newnameTestFolder);


$oldnameBcFolder = $familyName."\\".$productName."\\".$buildConfigOld;
$newnameBcFolder = $familyName."\\".$productName."\\".$buildConfigName;
rename("..\\..\\project_build\\".$oldnameBcFolder, "..\\..\\project_build\\".$newnameBcFolder);


$sqlProjName = mysqli_query($con,"SELECT * FROM buildconfig WHERE id = '$id'");
$row = mysqli_fetch_array($sqlProjName);
$projectIdDB = $row['projectId'];
$familyIdDB = $row['familyId'];

$sqlProjName = mysqli_query($con,"SELECT * FROM project WHERE id = '$projectIdDB'");
$row = mysqli_fetch_array($sqlProjName);
$productNameNew = $row['productName'];

$sqlProjName = mysqli_query($con,"SELECT * FROM masterfamily WHERE id = '$familyIdDB'");
$row = mysqli_fetch_array($sqlProjName);
$familyNameNew = $row['familyName'];



  $source = "..\\..\\project_build\\".$familyName."\\".$productNameOld."\\".$buildConfigOld;
//echo "\r\n";
   $destination = "..\\..\\project_build\\".$familyNameNew."\\".$productNameNew;
  //."\\".$productNameNew."\\".$buildConfigName;
  //$destination = "..\\..\\project_build\\ThorKA-N1100FZ";
	recurse_copy($source, $destination);
	
//echo $oldnameProductFolder = $familyName."\\".$productNameOld;
//echo "\r\n";
//echo $newnameProductFolder = $familyName."\\".$productNameNew;
//rename("..\\..\\project_build\\".$oldnameProductFolder, "..\\..\\project_build\\".$newnameProductFolder);
